...
void main() {
  group('RecurringTemplate — fromJson/toJson', () {
    test(
      'membaca kategori tersemat dan tanggal nullable dari contoh kontrak',
      () {
    ...
    test(
      'end_date terisi dan next_date null (template nonaktif) ikut round trip',
      () {
        final json = {
          ..._sampleTemplate,
          'end_date': '2027-03-01',
  ...
  group('recurringFirstOnOrAfter', () {
    test(
      'MONTHLY: 31 Jan dijepit ke akhir bulan, selalu dihitung dari anchor',
      () {
    ...
    test('kejadian pertama pada atau setelah hari ini', () {
      ...
      final anchor = DateTime(today.year, today.month, today.day - 1);
      final next = recurringFirstOnOrAfter(
        anchor,
        RecurringFrequency.weekly,
        today,
      );
      expect(
        next.isBefore(DateTime(today.year, today.month, today.day)),
        isFalse,
      );
  ...
  group('MockRecurringRepository', () {
    ...
    test(
      'create: next_date = kejadian pertama pada/setelah hari ini, aktif',
      () async {
        final start = DateTime.now().subtract(const Duration(days: 400));
        final tmpl = await repo.createTemplate(
          _draft(startDate: Tanggal.api(start)),
        );
        final expectedNext = recurringFirstOnOrAfter(
          start,
          RecurringFrequency.monthly,
          DateTime.now(),
        );
    ...
    test(
      'create: start_date di masa depan tidak dimundurkan (tanpa backfill)',
      () async {
    ...
    test(
      'create: end_date sudah lewat sejak awal → nonaktif, next_date null',
      () async {
        final start = DateTime.now().subtract(const Duration(days: 400));
    ...
    test(
      'update tanpa mengubah frekuensi/start_date mempertahankan next_date lama',
      () async {
    ...
    test('mengedit template yang sudah nonaktif → ApiException 409', () async {
      ...
      expect(
        () => repo.updateTemplate(
          created.id,
          _draft(startDate: created.startDate),
        ),
        throwsA(
    ...
    test(
      'stop pada template yang sudah nonaktif mengembalikannya apa adanya',
      () async {
        final start = DateTime.now().subtract(const Duration(days: 400));
        final end = DateTime.now().subtract(const Duration(days: 5));
        final created = await repo.createTemplate(
    ...
    test(
      'getTemplates: aktif lebih dulu, lalu next_date makin dekat',
      () async {
        final now = DateTime.now();
        final near = await repo.createTemplate(
          _draft(
            frequency: RecurringFrequency.weekly,
            startDate: Tanggal.api(now.subtract(const Duration(days: 3))),
          ),
        );
        final far = await repo.createTemplate(
          _draft(
            frequency: RecurringFrequency.weekly,
            startDate: Tanggal.api(now.add(const Duration(days: 20))),
          ),
        );
        final inactive = await repo.createTemplate(
          _draft(
  ...
  group('ApiRecurringRepository (kontrak REST)', () {
    ...
    test(
      'POST /recurring mengirim draft persis, membaca template dari respons',
      () async {
    ...
    test(
      'PATCH /recurring/{id} mengirim body penuh, sama bentuknya dengan POST',
      () async {
        useFakeServer({'template': _sampleTemplate});
        final draft = RecurringDraft(
          type: 'EXPENSE',
  ...
  group('supabaseException — validasi transaksi berulang (#58)', () {
    ...
    test('jenis dan metode pembayaran tidak dikenal', () {
      expect(
        map('23514', check('recurring_templates_type_check')).errors?.keys,
        ['type'],
      );
      expect(
        map(
          '23514',
          check('recurring_templates_payment_method_check'),
        ).errors?.keys,
        ['payment_method'],
      );
    });
    ...
    test('tanggal mulai dan berakhir di luar rentang 2000–2099', () {
      expect(
        map(
          '23514',
          check('recurring_templates_start_date_check'),
        ).errors?.keys,
        ['start_date'],
      );
      expect(
    ...
    test('tanggal berakhir sebelum tanggal mulai', () {
      final e = map(
        '23514',
        check('recurring_templates_end_after_start_check'),
      );
      expect(e.errors, {
    ...
    test('kategori tidak ada dan kategori salah jenis dibedakan', () {
      ...
      final salahJenis = map(
        '23503',
        fk('recurring_templates_category_type_fkey'),
      );
