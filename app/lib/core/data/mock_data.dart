...
class MockData {
  ...
  static List<TxCategoryData> get txCategories => const [
        ...
        TxCategoryData(id: 'ic5', name: 'Modal Masuk', type: 'INCOME', taxRelevant: false, isCogs: false, icon: '🏦', color: '#0891B2'),
        TxCategoryData(id: 'ic6', name: 'Pinjaman Diterima', type: 'INCOME', taxRelevant: false, isCogs: false, icon: '🤝', color: '#0891B2'),
        TxCategoryData(id: 'ic7', name: 'Hibah & Bantuan', type: 'INCOME', taxRelevant: false, isCogs: false, icon: '🎁', color: '#0891B2'),
        // Sewa tanah/bangunan, bunga bank, jasa konstruksi: sudah final tersendiri
        // (PMK 164/2023 Ps. 3 ayat 3 huruf c).
        TxCategoryData(id: 'ic8', name: 'Penghasilan Final Lainnya', type: 'INCOME', taxRelevant: false, isCogs: false, icon: '📄', color: '#0891B2'),

        // EXPENSE
        TxCategoryData(id: 'ec1', name: 'Bahan Baku', type: 'EXPENSE', taxRelevant: false, isCogs: true, icon: '📦', color: '#DC2626'),
        TxCategoryData(id: 'ec2', name: 'Barang Dagangan', type: 'EXPENSE', taxRelevant: false, isCogs: true, icon: '🏪', color: '#DC2626'),
        // TODO(Q-47-4): upah produksi langsung seharusnya HPP; kategori ini belum memisahkan.
        TxCategoryData(id: 'ec3', name: 'Gaji Karyawan', type: 'EXPENSE', taxRelevant: false, isCogs: false, icon: '👥', color: '#F59E0B'),
        TxCategoryData(id: 'ec4', name: 'Sewa Tempat', type: 'EXPENSE', taxRelevant: false, isCogs: false, icon: '🏠', color: '#F59E0B'),
        ...
        TxCategoryData(id: 'eca', name: 'Potongan Penjualan', type: 'EXPENSE', taxRelevant: false, isCogs: false, icon: '🏷️', color: '#9CA3AF'),
        TxCategoryData(id: 'ec0', name: 'Pajak Dibayar', type: 'EXPENSE', taxRelevant: false, isCogs: false, icon: '🧾', color: '#9CA3AF'),
  ...
  static List<TxData> get transactions {
    ...
    return [
      TxData(id: 't1', businessId: 'b1', date: now.subtract(const Duration(days: 1)), type: 'INCOME', amount: 5200000, category: _cat('ic1'), description: 'Penjualan produk online', paymentMethod: 'QRIS', createdAt: now),
      TxData(id: 't2', businessId: 'b1', date: now.subtract(const Duration(days: 2)), type: 'EXPENSE', amount: 2800000, category: _cat('ec1'), description: 'Restock bahan baku kain', paymentMethod: 'TRANSFER', createdAt: now),
      TxData(id: 't3', businessId: 'b1', date: now.subtract(const Duration(days: 5)), type: 'EXPENSE', amount: 9000000, category: _cat('ec3'), description: 'Gaji 3 karyawan Oktober', paymentMethod: 'TRANSFER', createdAt: now),
      TxData(id: 't4', businessId: 'b1', date: now.subtract(const Duration(days: 6)), type: 'INCOME', amount: 8500000, category: _cat('ic2'), description: 'Order custom seragam', paymentMethod: 'TRANSFER', createdAt: now),
      TxData(id: 't5', businessId: 'b1', date: now.subtract(const Duration(days: 7)), type: 'EXPENSE', amount: 620000, category: _cat('ec5'), description: 'PLN Oktober', paymentMethod: 'TRANSFER', createdAt: now),
      TxData(id: 't6', businessId: 'b1', date: now.subtract(const Duration(days: 9)), type: 'INCOME', amount: 3200000, category: _cat('ic1'), description: 'Penjualan batik tulis', paymentMethod: 'CASH', createdAt: now),
      // Perbaikan: sebelumnya memakai cats[5] (Barang Dagangan) untuk tagihan internet.
      TxData(id: 't7', businessId: 'b1', date: now.subtract(const Duration(days: 12)), type: 'EXPENSE', amount: 1500000, category: _cat('ec6'), description: 'Internet Biznet Oktober', paymentMethod: 'TRANSFER', createdAt: now),
      TxData(id: 't8', businessId: 'b1', date: now.subtract(const Duration(days: 14)), type: 'INCOME', amount: 12000000, category: _cat('ic2'), description: 'Proyek website client', paymentMethod: 'TRANSFER', createdAt: now),
      // Contoh non-omzet: tidak boleh menambah akumulasi ambang Rp500 juta,
      // ytdOmzet, maupun pkpPercent.
      TxData(id: 't9', businessId: 'b1', date: now.subtract(const Duration(days: 20)), type: 'INCOME', amount: 15000000, category: _cat('ic6'), description: 'Pencairan pinjaman modal kerja', paymentMethod: 'TRANSFER', createdAt: now),
    ];
  }
  ...
  static List<TaxDeadline> get deadlines {
    ...
    return [
      ...
      TaxDeadline(id: '3', label: 'SPT Tahunan OP ${now.year}', taxType: 'SPT', deadline: DateTime(now.year + 1, 3, 31), status: 'PENDING'),
    ];
  }
